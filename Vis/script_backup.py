...
def GetPolygon(n=5, gl = None, r = 15, spin = 0, mode = 0, edges = True):
    ...
    return GetShape(poly, gl = gl, r = r, spin = spin, mode = mode, edges = edges)
...
def GetShape(shape, gl = None, r = 15, spin = 0, mode = 0, edges = True, bold = .05):
        ...
        if edges:
            mx = gl.GLMeshItem(vertexes=verts, faces=faces, faceColors=colors, smooth=False ,drawEdges=True, edgeColor=(1, 1, 0, 0.3))
        else:
            mx = gl.GLMeshItem(vertexes=verts, faces=faces, faceColors=colors, smooth=False)
...
#    widgett.show()
    ...
    for l in range(layers):
        polyg, poly = GetPolygon(n,gl, r = R/layers*(l+1), spin = (360/n)*l*spin , mode = 2)
        polyList.append(poly)
        polyg.translate(0, 0, ((layers-1)-l)*dist)
        w.addItem(polyg)
    ...
    for ns in range(n):
        ...
        vc+=1
    mn, mshape = GetShape(newShape, gl = gl, r = R, spin = spin, mode = 2, edges = True, bold = 1)
    mn.translate(0, 0, 10)
    w.addItem(mn)
